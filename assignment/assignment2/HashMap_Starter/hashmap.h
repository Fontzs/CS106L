...
public:
     ...
     *      auto [iter1, insert1] = map.insert({3, "Avery"}); // inserts {3, "Avery"}, iter1 points
     * to that element, insert1 = true auto [iter2, insert2] = map.insert({3, "Anna"});  // no-op,
     * iter2 points to {3, "Avery"}, insert2 = false
     *
     ...
     *
     * Notes: you may want to do some research on initializer_lists. The most important detail you
     * need to know is that they are very limited, and have three functions: init.begin(),
     * init.end(), and init.size(). There are no other ways to access the elements in an
     * initializer_list. As a result, you probably want to leverage the range constructor you wrote
     * in the previous function!
     *
     * Also, you should check out the delegating constructor note in the .cpp file.
     */
    HashMap(std::initializer_list<value_type> init, size_t bucket_count = kDefaultBuckets,
            const H& hash = H());
...
private:
    ...
    struct node {
        ...
        /*
         * Constructor with default values, so even if you forget to set next to nullptr it'll be
         * fine.
         *
         * Usage:
     ...
     *      const auto& [key, mapped] = ptr->value;     // each node* contains a value that is a
     * pair
     */
