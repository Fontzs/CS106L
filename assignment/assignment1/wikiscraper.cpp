...
unordered_set<string> findWikiLinks(const string& inp) {
    ...
    static const string delim = "href=\"/wiki/";  // prefix of HTML

    unordered_set<string> ret;  // return set

    auto url_start = inp.begin();  // begin
    auto end = inp.end();          // end

    while (true) {
        // TODO: ASSIGNMENT 2 TASK 1:
        // Set url_start to the next location of "delim" (starting your search at url_start), using
        // std::search. After doing so, break out of the while loop if there are no occurrences of
        // delim left (use your work from the line above). Estimated length: 2-3 lines
        ...
        // Set url_end to the end of the wikilink. Start searching after the delimeter you found
        // above. Make sure to use std::find! (std::find looks for a single element in a container,
        // e.g. character in a string—std::search looks for a series of elements in a container,
        // like a substring in a string. remember that a string is represented as an array of
        // characters, and is also a container!) Estimated length: 1 lines
        ...
        // Last exercise of this function! Create a string from the two iterators (url_start and
        // url_end) above using a string constructor. Make sure you start the string AFTER the
        // delimiter you found in task 5! Estimated length: 1 lines
...
string WikiScraper::getPageSource(const string& page_name) {
    ...
    if (page_cache.find(page_name) == page_cache.end()) {
        ...
        if (r.status_code != 200) {
            notFoundError("Couldn't get page source. Have you entered a valid link?", page_name,
                          url);
            return "";
        }
